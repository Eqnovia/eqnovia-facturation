/**
 * PROFORMA - Gestion des factures pro forma
 */
const ProForma = {
    KEY: Database.KEYS.PROFORMA,
    docType: 'FACTURE PRO FORMA',
    dbNumberType: 'proforma',
    ...
    supprimer(id) {
        ...
        this.afficher();
        Toast.success('Facture Pro Forma supprimée avec succès');
    },
    ...
    afficher() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = '<div class="empty-state">Aucune pro forma trouvée</div>';
            return;
        }

        let html = `<table class="data-table"><thead><tr><th>Réf.</th><th>Client</th><th>Date</th><th>Total TTC</th><th>Actions</th></tr></thead><tbody>`;
        filtered.forEach(d => {
            html += `<tr>
    ...
    getFormHtml(doc) {
        ...
        return `
            <form id="proforma-form" onsubmit="return ProForma.sauvegarder(event)">
    ...
    convertirFacture(id) {
        const proforma = this.getById(id);
        if (!proforma) return Toast.error('Pro forma introuvable');
        const factureData = { ...proforma };
